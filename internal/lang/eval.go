...
// checkForUnknownFunctionDiags inspects the diagnostics for errors
// which may indicate a function was used from a provider which has not been
// included in the configuration. If such an error is found, we replace the
// diagnostic with something more specific to Terraform.
